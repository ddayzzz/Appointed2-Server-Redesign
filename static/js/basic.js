    ...
    $.fn.extend({
        showFormError: function (err) {
            return this.each(function () {
                ...
                if (err) {
                    $alert.text((err.message ? err.message : (err.error ? err.error : err)) + ", 服务器端的消息：" + (err.origin_message ? err.origin_message : '<无>')).removeClass('uk-hidden').show();  //可以显示Appointed2发送的错误
...
function _httpJSON(method, url, data, callback) {
    ...
    $.ajax(opt).done(function (r) {
        ...
        {
            org_message = jqXHR.responseJSON.messages.join(",");
        }

        return callback({'error': 'http_bad_response', 'data': '' + jqXHR.status, 'message': '网络好像出问题了 (HTTP ' + jqXHR.status + ')', 'origin_message':
        org_message});
    });
}
